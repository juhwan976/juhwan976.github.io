...
import * as S from "./LiquidEther.styles";
...
export interface LiquidEtherProps {
  ...
  /** 첫 프레임이 그려진 직후 1회 호출 — 로딩 인디케이터 해제용 */
  onFirstFrame?: () => void;
}
...
export default function LiquidEther({
  ...
  // 콜백 변경으로 WebGL 컨텍스트가 재생성되지 않도록 ref로 우회한다.
...
`;
    ...
    type RenderTargetLike = THREE.WebGLRenderTarget | null | undefined;

    interface ShaderPassProps {
      material?: THREE.ShaderMaterialParameters;
      output?: RenderTargetLike;
      output0?: RenderTargetLike;
      output1?: RenderTargetLike;
    }

    /** 시뮬레이션 패스 생성에 쓰이는 공용 인자 */
    interface SimPassProps {
      ...
      cursor_size?: number;
      viscous?: number;
      src?: RenderTargetLike;
      dst?: RenderTargetLike;
      dst_?: RenderTargetLike;
      src_p?: RenderTargetLike;
      src_v?: RenderTargetLike;
    }
    ...
    class ShaderPass {
      ...
      update(..._args: unknown[]): RenderTargetLike | void {
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: unknown[]): RenderTargetLike {
        ...
        if (!this.uniforms) return;
        let fbo_in: RenderTargetLike;
        let fbo_out: RenderTargetLike;
    ...
    class Divergence extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { vel } = (args[0] || {}) as { vel?: RenderTargetLike };
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: unknown[]): RenderTargetLike {
        ...
        let p_in: RenderTargetLike;
        let p_out: RenderTargetLike;
        const iter = iterations ?? 0;
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { vel, pressure } = (args[0] || {}) as {
          vel?: RenderTargetLike;
          pressure?: RenderTargetLike;
        };
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      loop() {
        ...
        this.render();
        if (!firstFrameNotifiedRef.current) {
  ...
  return <S.Container ref={mountRef} style={style} />;
}
